...
float** rotate180(float** mat, nSize matSize)// rotate 180°
{
	int i,c,r;
...
float** correlation(float** map,nSize mapSize,float** inputData,nSize inSize,int type)// �����
{
	...
	int halfmapsizew;
	int halfmapsizeh;
	if(mapSize.r%2==0&&mapSize.c%2==0){ // size of kernel is even
		halfmapsizew=(mapSize.c)/2; // half of the kernel
		halfmapsizeh=(mapSize.r)/2;
	}
	else{
		halfmapsizew=(mapSize.c-1)/2; // half of the kernel
