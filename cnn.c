...
CovLayer* initCovLayer(int inputWidth,int inputHeight,int mapSize,int inChannels,int outChannels)
{
	...
	covL->isFullConnect=true; // is full-connect by default
...
OutLayer* initOutLayer(int inputNum,int outputNum)
{
	...
	for(i=0;i<outputNum;i++){
		...
		for(j=0;j<inputNum;j++){
			float randnum=(((float)rand()/(float)RAND_MAX)-0.5)*2; // produce a random number between -1 to 1
...
int vecmaxIndex(float* vec, int veclength)// return the index with the max value
{
	int i;
	float maxnum=-1.0;
	int maxIndex=0;
...
{
	int n=0;
	int incorrectnum=0;  //number of incorrect predictions
	for(n=0;n<testNum;n++){
		cnnff(cnn,inputData->ImgPtr[n].ImgData);
		printf("testing process : %d / %d",n,testNum);
...
void predict(CNN* cnn, ImgArr inputData,int testNum) //predict what the digit is 
{
	int n=0;	
	for(n=0;n<testNum;n++){
		cnnff(cnn,inputData->ImgPtr[n].ImgData);
...
void importcnn(CNN* cnn, const char* filename)
{
		...
		for(j=0;j<cnn->C1->outChannels;j++)
			for(r=0;r<cnn->C1->mapSize;r++)
				for(c=0;c<cnn->C1->mapSize;c++){
					fread(&cnn->C1->mapData[i][j][r][c],sizeof(float),1,fp);
					printf("%5.2f",cnn->C1->mapData[i][j][r][c]);
				}
		printf("\n");
	}
	printf("\nc1 basic data\n");
	for(i=0;i<cnn->C1->outChannels;i++){
		...
		printf("%5.2f ",cnn->C1->basicData[i]);
	}
		...
		for(j=0;j<cnn->C3->outChannels;j++)
			for(r=0;r<cnn->C3->mapSize;r++)
				for(c=0;c<cnn->C3->mapSize;c++){
					...
					printf("%5.2f ",cnn->C3->mapData[i][j][r][c]);
				}
		printf("\n");
	}
	printf("\nc3 basicdata\n");
	...
	for(i=0;i<cnn->C3->outChannels;i++){
		...
		printf("%5.2f ",cnn->C3->basicData[i]);
	}
	...
	for(i=0;i<cnn->O5->outputNum;i++){
		...
		printf("%5.2f ",cnn->O5->basicData[i]);	
	}
...
void cnntrain(CNN* cnn,	ImgArr inputData,LabelArr outputData,CNNOpts opts,int trainNum)
{
	...
	for(e=0;e<opts.numepochs;e++){
		...
		for(n=0;n<trainNum;n++){
			//printf("%d\n",n);
			printf("training process: %d / %d\n",n,trainNum);
			cnnff(cnn,inputData->ImgPtr[n].ImgData);  //feedforward
			cnnbp(cnn,outputData->LabelPtr[n].LabelData); // backpropagation, calculates local gradients

			FILE *fp=NULL;
			char* filedir="C:\\Users\\79993\\Desktop\\DeepLearningC-master\\CNN\\CNN\\CNNdata\\";
			const char* filename=combine_strings(filedir,combine_strings(intTochar(n),".dat"));
			fp=fopen(filedir,"r");
			if (fp==NULL)
				fp=fopen(filedir,"w");
			fclose(fp);
			savecnndata(cnn,filename,inputData->ImgPtr[n].ImgData);
			cnnapplygrads(cnn,opts,inputData->ImgPtr[n].ImgData); // update gradients
...
void cnnff(CNN* cnn,float** inputData)
{
	...
	// output of the full-connect layer
	//flatten the data into one-dimentional vector
...
float activation_Sigma(float input,float bas) // sigma function
{
	float temp=input+bas;
	return (float)1.0/((float)(1.0+exp(-temp)));
}

void avgPooling(float** output,nSize outputSize,float** input,nSize inputSize,int mapSize) //average pooling
{
...
// full-connect layer feedforward
...
{	
	...
	// gradient doesn't change because of average pooling
	for(i=0;i<cnn->C1->outChannels;i++){
...
{
	// update weights data for cov layer and output layer
	int i,j,r,c;
	...
	// output layer
	// flatten into one-dimentional vector
	float* O5inData=(float*)malloc((cnn->O5->inputNum)*sizeof(float)); 
	nSize outSize={cnn->S4->inputWidth/cnn->S4->mapSize,cnn->S4->inputHeight/cnn->S4->mapSize};
	for(i=0;i<(cnn->S4->outChannels);i++)
		for(r=0;r<outSize.r;r++)
